...
class TestSwitchAttrName(unittest.TestCase):
    ...
    def test_name_write_beyondmax(self):
        ...
        with self.assertRaises(CLIError) as e:
            self.switch.name = name
        self.assertEqual("The command \" switchname "+str(name)+" \" gave the error \" % String exceeded max length of (32) \".",str(e.exception))

    def test_name_write_invalid(self):
        name = self.invalid_name # starts with digit
        with self.assertRaises(CLIError) as e:
            self.switch.name = name
        self.assertEqual("The command \" switchname "+str(name)+" \" gave the error \" Invalid switch name. Must start with a letter, end with alphanumeric\nand contain alphanumeric and hyphen only \".",str(e.exception))
