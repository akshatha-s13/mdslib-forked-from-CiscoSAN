...
class TestZoneCreate(unittest.TestCase):
    ...
    def test_nonexistingvsan(self):
        ...
        self.assertEqual("VsanNotPresent: Vsan "+str(i)+" is not present on the switch. Please create the vsan first.",str(e.exception))
    ...
    def test_create_name_invalid(self):
        ...
        with self.assertRaises(CLIError) as e:
            z.create()
        self.assertEqual("The command \" zone name "+str(name)+" vsan "+str(i)+" \" gave the error \" Illegal character present in the name \".",str(e.exception))
        v.delete()
    ...
    def test_create_name_invalidfirstchar(self):
        ...
        with self.assertRaises(CLIError) as e:
            z.create()
        self.assertEqual("The command \" zone name "+str(name)+" vsan "+str(i)+" \" gave the error \" Illegal first character (name must start with a letter) \".",str(e.exception))
        v.delete()
    ...
    def test_create_name_beyondmax(self):
        ...
        with self.assertRaises(CLIError) as e:
            z.create()
        self.maxDiff = 1000
        self.assertEqual("The command \" zone name "+ str(name) +" vsan "+str(i)+" \" gave the error \" % String exceeded max length of (64) \".",str(e.exception))
        v.delete()
