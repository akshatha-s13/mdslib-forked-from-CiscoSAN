...
class TestDeviceAliasRename(unittest.TestCase):
    ...
    def test_rename_existingnew(self):
        ...
        with self.assertRaises(CLIError) as e:
            d.rename(oldname=oldname, newname=newname)
        self.assertEqual('The command " device-alias database ; device-alias rename '+str(oldname)+' '+str(newname)+' " gave the error " Target device-alias name already in use. Please specify a new name.\nPlease use \'show device-alias session rejected\' to display the rejected set of commands and for the device-alias best-practices recommendation. ".',str(e.exception))
    ...
    def test_rename_nonexistingold(self):
        ...
        with self.assertRaises(CLIError) as e:
            d.rename(oldname=oldname, newname=newname)
        self.assertEqual('The command " device-alias database ; device-alias rename '+str(oldname)+' '+str(newname)+' " gave the error " Device Alias not present\nPlease use \'show device-alias session rejected\' to display the rejected set of commands and for the device-alias best-practices recommendation. ".',str(e.exception))
    ...
    def test_rename_emptydb(self):
        ...
        self.assertEqual('The command " device-alias database ; device-alias rename da2 da3 " gave the error " Device Alias not present\nPlease use \'show device-alias session rejected\' to display the rejected set of commands and for the device-alias best-practices recommendation. ".',str(e.exception))
