...
class TestVsanAttrName(unittest.TestCase):
    ...
    def test_name_write_beyondmax(self):
        ...
        with self.assertRaises(CLIError) as e:
            v.name = name
        self.assertEqual('The command " vsan database ; vsan '+str(self.vsan_id[4])+' name \''+str(name)+'\' " gave the error " % String exceeded max length of (32) ".',str(e.exception))
        v.delete()
    ...
    def test_name_write_specialchar(self):
        ...
        with self.assertRaises(CLIError) as e:
            v.name = name
        self.assertEqual('The command " vsan database ; vsan '+str(i)+' name \'' + str(name) + '\' " gave the error " Request contains invalid special characters ".', str(e.exception))
        v.delete()
    ...
    def test_name_write_boundary(self):
        for i in self.boundary_id:
            ...
            with self.assertRaises(CLIError) as e:
                v.name = 'vsan'
            self.assertEqual('The command " vsan database ; vsan '+str(i)+' name \'vsan\' " gave the error " % Invalid command ".',str(e.exception))
    ...
    def test_name_write_reserved(self):
        for i in self.reserved_id:
            ...
            with self.assertRaises(CLIError) as e:
                v.name = 'vsan'
            self.assertEqual('The command " vsan database ; vsan '+str(i)+' name \'vsan\' " gave the error " vsan '+str(i)+':vsan(s) reserved ".',str(e.exception))
