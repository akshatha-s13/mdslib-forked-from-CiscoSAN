...
class TestVsanAttrSuspend(unittest.TestCase):
    ...
    def test_suspend_write_boundary(self):
        for i in self.boundary_id:
            ...
            with self.assertRaises(CLIError) as e:
                v.suspend = True
            self.assertEqual('The command " vsan database ; vsan '+str(i)+' suspend " gave the error " % Invalid command ".',
                             str(e.exception))
    ...
    def test_suspend_write_reserved(self):
        for i in self.reserved_id:
            ...
            with self.assertRaises(CLIError) as e:
                v.suspend = True
            self.assertEqual('The command " vsan database ; vsan '+str(i)+' suspend " gave the error " vsan '+str(i)+':vsan(s) reserved ".',
                             str(e.exception))
